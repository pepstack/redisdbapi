...
#***********************************************************
# file_chksum
#   check md5sum of file $1 is equal to $2
#
# Example:
#   ret=$(file_cksum $FILE $MD5)
#   if [ $ret = "0" ]; then
#     echo "check md5sum is ok"
#   elif [ $ret = "1" ]; then
#     echo "check md5sum is bad"
#   else
#     echo "file not found"
#   fi
#***********************************************************
function file_cksum() {
    if [ -f "$1" ]; then
        local md5=$(echo $(md5sum "$1") | awk '{print $1}')
        if [ "$md5" = "$2" ]; then
            echo "0"
        else
            echo "1"
