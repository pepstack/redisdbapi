...
# 执行查找并处理
# files=$(find "$_thisdir" $(build_ignore_prune) -type f "${src_conditions[@]}" -print0 | xargs -0 -I{} echo "{}")
mapfile -d '' fileslist < <(find "$_thisdir" $(build_ignore_prune) -type f "${src_conditions[@]}" -print0)
